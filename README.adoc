...
It should not be viewed as a standalone AsciiDoc processor implementation.
Rather, it's only meant to serve as a reference.
...
Up until now, the AsciiDoc syntax rules have only been described informally through implementation code and user-oriented documentation.
...
Establishing a formal grammar is a major step forward for AsciiDoc and the specification.
It will help root out inconsistencies, ambiguities, and idiosyncrasies.
...
Thus, in order to work with it, you first need to make sure you have Node.js installed.
...
The parsers are generated from grammar files that are located in the _grammar_ folder.
...
Some tests also have a configuration file that ends in _-config.yml_).
