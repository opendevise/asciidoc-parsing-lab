...
module.exports = (input, { attributes, mode, sourceMapping } = {}) => {
  ...
  if (replaceArgs) {
    ...
    ;(sourceMapping ??= [...Array(input.length)].map((_, offset) => ({ offset }))).offset = 0
    const preprocessedInput = input.replace.apply(input, replaceArgs)
    delete sourceMapping.offset
    if (modified) return { input: preprocessedInput, sourceMapping }
    sourceMapping = providedSourceMapping
  }
